...
void Fbx::Draw(Transform& transform, SHADER_TYPE shaderType,int frame,XMFLOAT2 uvScroll)
{
	...
	for (int i = 0; i<parts_.size(); i++)
	{
		...
		{
			parts_[i]->DrawSkinAnime(transform, frame,uvScroll);
		}
		else
		{
			parts_[i]->Draw(transform,uvScroll);
		}
	}
}

void Fbx::DrawOutLine(Transform& transform, int frame, XMFLOAT2 uvScroll)
{
	Direct3D::SetBlendMode(BLEND_DEFAULT);
...
void Fbx::DrawToon(Transform& transform, bool isOutLine, int frame, XMFLOAT2 uvScroll)
{
	Direct3D::SetBlendMode(BLEND_DEFAULT);
	if (isOutLine)
	{
